...
async function getLatestVersionForBenefit(db: ReturnType<typeof getDb>, benefitId: string): Promise<number> {
	const rows = await db.select().from(eligibility_rules).where(eq(eligibility_rules.benefit_id, benefitId)).all();
...
async function getNextPriorityForVersion(db: ReturnType<typeof getDb>, benefitId: string, targetVersion: number): Promise<number> {
	const rows = await db
		.select()
	...
	const versionRows = rows.filter((row) => normalizeStoredRuleValue(row.value).version === targetVersion);

	if (versionRows.length === 0) return 1;

	return versionRows.reduce((maxPriority, row) => Math.max(maxPriority, row.priority), 0) + 1;
}
...
function buildStoredRuleValue(params: {
	...
	const parsedInputValue = params.value !== undefined && params.value !== null ? parseRuleInputValue(params.value) : params.base.value;

	if (isRecord(parsedInputValue)) {
		return normalizeStoredRuleValue({
			...parsedInputValue,
			type: params.type ?? parsedInputValue.type ?? params.base.type,
			operator: params.operator ?? parsedInputValue.operator ?? params.base.operator,
			version: params.configVersion ?? parsedInputValue.version ?? params.base.version,
...
export const eligibilityRuleMutation = {
	Mutation: {
		createEligibilityRule: async (
			...
			const nextPriority = input.priority ?? (await getNextPriorityForVersion(db, input.benefitId, storedRuleValue.version));
		...
		updateEligibilityRule: async (
			...
			const db = getDb(context.env.DB);
			const { input } = args;
			const existing = await db.select().from(eligibility_rules).where(eq(eligibility_rules.id, input.id)).get();
			...
			const updated = await db
				...
				.set({
					...(input.value !== undefined || input.type !== undefined || input.operator !== undefined || input.configVersion !== undefined
						? { value: nextValue }
						: {}),
					...(input.errorMessage !== undefined ? (input.errorMessage != null ? { error_message: input.errorMessage } : {}) : {}),
		...
		deleteEligibilityRule: async (_parent: unknown, args: { id: string }, context: { env: Env }) => {
			const db = getDb(context.env.DB);
			const deleted = await db.delete(eligibility_rules).where(eq(eligibility_rules.id, args.id)).returning().get();
