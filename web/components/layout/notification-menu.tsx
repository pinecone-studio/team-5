...
const REVIEWER_NOTIFICATIONS_QUERY = gql`
  query HeaderReviewerNotifications {
    employees {
      id
      fullName
      email
...
interface ReviewerNotificationsQueryData {
  employees: Array<{
    id: string;
    fullName: string;
    email: string;
...
function getEmployeeMessage(status: string, benefitName: string) {
  switch (status) {
    case "approved":
      return {
        lead: "",
        highlight: benefitName,
        tail: " request is approved.",
      };
    case "rejected":
      return {
        lead: "",
        highlight: benefitName,
        tail: " request has been rejected.",
      };
    case "cancelled":
      return {
        lead: "",
        highlight: benefitName,
        tail: " request was cancelled.",
      };
    default:
      return {
        lead: "",
        highlight: benefitName,
        tail: " request is pending review.",
      };
  }
}

function getReviewerMessage(
  employeeName: string,
  status: string,
  benefitName: string,
) {
  switch (status) {
    case "pending":
      return {
        lead: `${employeeName} submitted a request for the `,
        highlight: benefitName,
        tail: " benefit.",
      };
    case "approved":
      return {
        lead: `${employeeName} `,
        highlight: benefitName,
        tail: " request was approved.",
      };
    case "rejected":
      return {
        lead: `${employeeName} `,
        highlight: benefitName,
        tail: " request was rejected.",
      };
    case "cancelled":
      return {
        lead: `${employeeName} `,
...
function NotificationMenuContent({
  ...
  const { data: reviewerData, loading: reviewerLoading } =
    useQuery<ReviewerNotificationsQueryData>(REVIEWER_NOTIFICATIONS_QUERY, {
      skip: !isManagerShell,
      pollInterval: 20000,
      fetchPolicy: "cache-and-network",
    });
  ...
  const notifications = useMemo<NotificationEntry[]>(() => {
    if (isEmployeeShell) {
      return [...(employeeData?.myRequests ?? [])]
        ...
        .map(({ benefit, request }) => ({
          id: request.id,
          ...getEmployeeMessage(request.status, benefit.name),
          seenKey: `${request.id}:${request.updatedAt}`,
          timeLabel: formatRelativeTime(request.updatedAt),
        }));
    }

    const reviewData = isFinanceReviewer ? financeData : reviewerData;
    if (isFinanceReviewer || isManagerShell) {
      const employeeMap = new Map(
        (reviewData?.employees ?? []).map((employee) => [
          employee.id,
          employee.fullName?.trim() || employee.email || "Employee",
        ]),
      );
      const benefitMap = new Map(
        (reviewData?.benefits ?? []).map((benefit) => [benefit.id, benefit.name]),
      );

      return [...(reviewData?.benefitRequests ?? [])]
        .sort(
          (leftItem, rightItem) =>
        ...
        .map((request) => {
          ...
          const employeeName =
            employeeMap.get(request.employeeId) ?? "Employee";

          return {
            id: request.id,
            ...getReviewerMessage(employeeName, request.status, benefitName),
            seenKey: `${request.id}:${request.updatedAt}`,
            timeLabel: formatRelativeTime(request.updatedAt),
          };
        });
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open ? (
        <div className="absolute right-0 top-full z-50 mt-3 w-[min(30.625rem,calc(100vw-1.5rem))] overflow-hidden rounded-[12px] border border-[#d9e1ef] bg-white shadow-[0_16px_38px_rgba(15,23,42,0.12)]">
          <div className="border-b border-[#e8edf5] px-4 py-3">
            <p className="text-[1rem] font-semibold leading-6 text-[#17243d]">
              Notification
            </p>
          </div>
          ...
          <div className="max-h-[18rem] overflow-y-auto">
            {isLoading ? (
              <div className="px-4 py-4 text-sm text-[#708198]">
              ...
              notifications.map((item, index) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-[0.98rem] leading-8 text-[#3f4b63]">
                      {item.lead}
                      {item.highlight ? (
                        <span className="font-semibold text-[#17243d]">
                          {item.highlight}
                        </span>
                      ) : null}
                      {item.tail}
